...
conn = ibm_db.connect("DATABASE=bludb;HOSTNAME=21fecfd8-47b7-4937-840d-d791d0218660.bs2io90l08kqb1od8lcg.databases.appdomain.cloud;PORT=31864;SECURITY=SSL;SSLServerCertificate=DigiCertGlobalRootCA.crt;UID=gkx49901;PWD=kvWCsySl7vApfsy2", '', '')
app = Flask(__name__)
    ...
    if request.method == "POST":
        try:
            ...
            if dictionary:
                ...
                ibm_db.execute(pstmt)
                total_stock = 'SELECT  FROM stocks WHERE ID= ?'
                stmts = ibm_db.prepare(conn, total_stock)
                ibm_db.bind_param(stmts, 1, stock_id)
                ibm_db.execute(stmts)
                print(total_stock)
                edit_stock = 'UPDATE QUANTITY stocks SET ' +  + "= ?" + " WHERE NAME=?"
                pstmts = ibm_db.prepare(conn, edit_stock)
                print(total_stock-quantity)
                ibm_db.bind_param(pstmts, 1, total_stock - quantity)
                ibm_db.bind_param(pstmts, 2, stock_id)
                ibm_db.execute(pstmt)
        except Exception as e:
            print(e)
